...
/* Recursion: 
   ...
   A function/ procedure that calls itself ?
...
================================================================================
 ...
 void PrintNumbers(int N, int M) 
 {
     ...
     {
         cout << N << endl;              5       +----------------------+
         PrintNumbers(N + 1, M);                 | PrintNumbers(5,4)   |   <-- then this one becomes Active Frame when PrintNumbers(5, 4) is called (implementation executed).
     }                                           +----------------------+

 }                                       4       +----------------------+
                                                 | PrintNumbers(4,4)   |   <-- then this one becomes Active Frame when PrintNumbers(4, 4) is called (implementation executed).
                                                 +----------------------+
 ...
 // Crucial Reminder: 
    when main() becomes the active frame, the line(s) of code inside it if it/they exist will execute
    and then on the next line, the called function (in this case: PrintNumbers(1, 4)) becomes the active frame as it's executed.

    and the process continues till, PrintNumbers(5,4) is the last frame one on top of the stack.

    NOW:

    When PrintNumbers(5,4) frame becomes active -- >   if (false)
    --> controller exits the PrintNumbers() procedure in the program --> 
    and as soon as controller exists: Function  PrintNumbers(5,4)  popped off the stack.

    now, PrintNumbers(4,4) is what we have on the top of the stack  --> is there something to execute in the code ? 
    Answer is no, so  PrintNumbers(4,4) is popped off the stack.
...
/*
 Also, be wary of the following CODING ERROR (Calling the recursive function/procedure outside of the if statement).
 This results in an infinite loop that's in turn results in Stack overflow exception.
 
*/

// So understand what happens here just by examining the code snippet:
void PrintNumbers(int N, int M)
{
    if (N <= M)
    {
...
/*Now, we're told that recursive functions can be used with no issues and all is fine !
  ...
  and on top of that, a Call Stack has its limits. This is why you can not just use recursive functions without understanding.
  For example, 
...
Exception non gérée à 0x00007FFFF5590CF7 (ucrtbased.dll) dans 19.Recursion.exe  : 
0xC00000FD : Stack overflow(paramètres  : 0x0000000000000001, 0x00000090F2303000).
